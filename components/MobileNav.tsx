'use client'

import { useState } from 'react'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { useTranslations, useLocale } from 'next-intl'

import LanguageSwitcher from './LanguageSwitcher'

export default function MobileNav() {
  const [menuOpen, setMenuOpen] = useState(false)
  const t = useTranslations('mobileNav')
  const pathname = usePathname()
  const locale = useLocale()

  const isHomePage = pathname === '/' || pathname === `/${locale}` || pathname === '/mk' || pathname === '/en'

  const isActive = (path: string) => {
    if (path === '/') {
      return pathname === '/' || pathname === `/${locale}`
    }
    return pathname.includes(path)
  }

  const getLocalizedPath = (path: string) => {
    if (locale === 'mk') return path
    return `/${locale}${path}`
  }

  const getAnchorHref = (hash: string) => {
    if (isHomePage) return `#${hash}`
    return `${getLocalizedPath('/')}#${hash}`
  }

  return (
    <>
      {/* Bottom Navigation Bar */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-space-surface bg-space/95 backdrop-blur-sm dark:border-dark-space-surface dark:bg-dark-space/95 md:hidden">
        <div className="flex items-center justify-around py-2">
          {/* Home */}
          <Link
            href={getLocalizedPath('/')}
            className={`flex flex-col items-center px-3 py-2 transition-all ${
              isActive('/') && !isActive('/projects')
                ? 'text-accent-blue'
                : 'text-text-secondary dark:text-dark-text-secondary'
            }`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="h-6 w-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25"
              />
            </svg>
            <span className="mt-1 text-xs">{t('home')}</span>
          </Link>

          {/* Portfolio */}
          <a
            href={getAnchorHref('portfolio')}
            className="flex flex-col items-center px-3 py-2 text-text-secondary transition-all dark:text-dark-text-secondary"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="h-6 w-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z"
              />
            </svg>
            <span className="mt-1 text-xs">{t('portfolio')}</span>
          </a>

          {/* Services */}
          <a
            href={getAnchorHref('services')}
            className="flex flex-col items-center px-3 py-2 text-text-secondary transition-all dark:text-dark-text-secondary"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="h-6 w-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
              />
            </svg>
            <span className="mt-1 text-xs">{t('services')}</span>
          </a>

          {/* Contact */}
          <a
            href={getAnchorHref('contact')}
            className="flex flex-col items-center px-3 py-2 text-text-secondary transition-all dark:text-dark-text-secondary"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="h-6 w-6"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"
              />
            </svg>
            <span className="mt-1 text-xs">{t('contact')}</span>
          </a>

          {/* Menu (for theme/language) */}
          <button
            onClick={() => setMenuOpen(!menuOpen)}
            className={`flex flex-col items-center px-3 py-2 transition-all ${
              menuOpen ? 'text-accent-blue' : 'text-text-secondary dark:text-dark-text-secondary'
            }`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className={`h-6 w-6 transition-transform ${menuOpen ? 'rotate-90' : ''}`}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"
              />
            </svg>
            <span className="mt-1 text-xs">{t('menu')}</span>
          </button>
        </div>
      </nav>

      {/* Menu Overlay */}
      {menuOpen && (
        <>
          {/* Backdrop */}
          <div
            className="fixed inset-0 z-40 bg-space/80 backdrop-blur-sm dark:bg-dark-space/80 md:hidden"
            onClick={() => setMenuOpen(false)}
          />

          {/* Menu Panel */}
          <div className="fixed bottom-16 left-0 right-0 z-50 rounded-t-3xl border-t border-space-surface bg-space p-6 dark:border-dark-space-surface dark:bg-dark-space md:hidden">
            {/* Handle indicator */}
            <div className="mx-auto mb-6 h-1 w-12 rounded-full bg-space-surface dark:bg-dark-space-surface" />

            {/* Settings */}
            <div className="space-y-6">
              {/* Language Section */}
              <div className="flex items-center justify-between rounded-xl border border-space-surface bg-space-light p-4 dark:border-dark-space-surface dark:bg-dark-space-light">
                <div className="flex items-center gap-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="h-5 w-5 text-accent-blue"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418"
                    />
                  </svg>
                  <span className="text-sm font-medium text-text-primary dark:text-dark-text-primary">
                    Language
                  </span>
                </div>
                <LanguageSwitcher />
              </div>

              {/* Quick Links */}
              <div className="grid grid-cols-2 gap-3">
                <a
                  href={getAnchorHref('faq')}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center justify-center gap-2 rounded-xl border border-space-surface bg-space-light p-3 text-sm font-medium text-text-primary transition-all hover:border-accent-blue/50 dark:border-dark-space-surface dark:bg-dark-space-light dark:text-dark-text-primary"
                >
                  FAQ
                </a>
                <a
                  href={getAnchorHref('process')}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center justify-center gap-2 rounded-xl border border-space-surface bg-space-light p-3 text-sm font-medium text-text-primary transition-all hover:border-accent-blue/50 dark:border-dark-space-surface dark:bg-dark-space-light dark:text-dark-text-primary"
                >
                  Process
                </a>
              </div>
            </div>
          </div>
        </>
      )}
    </>
  )
}
